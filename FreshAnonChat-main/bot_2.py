# bot_2.py — RU/EN, без 18+, без сообщений о простое, c гашением TelegramForbiddenError
import asyncio
import logging
import os
import random
...
# ==== импорт БД API (как в твоём проекте) ====
...
# ==== опциональные dev-роутеры и команды ====
try:
    from dev_match_router import router_dev  # type: ignore
except Exception:
    router_dev = None
...
# ===== Вайбы/интересы (с подписью по языку) =====
VIBES_MAP = {
    "ru": {
        "funny": "😂 Весёлый",
        "calm": "🧘 Спокойный",
...
async def _cleanup_blocked_user(uid: int):
    """Если юзер заблокировал бота — снять с поиска/чата и уведомить вторую сторону."""
    try:
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    if not (user and isinstance(user[1], int) and user[1] >= 13 and user[2]):
        await state.clear()
        await safe_answer(message, tr("ru", "welcome"))  # стартовое — на RU, дальше выберем язык
...
# ===================== СЛУЖЕБНОЕ: КОМАНДЫ БОТА (fallback) =====================
async def _apply_default_bot_commands(bot: Bot):
    ru = [
        types.BotCommand(command="start", description="Запуск / перезапуск"),
        types.BotCommand(command="search", description="Поиск собеседника"),
        types.BotCommand(command="profile", description="Профиль"),
        types.BotCommand(command="settings", description="Настройки"),
        types.BotCommand(command="about", description="О Neverland"),
        types.BotCommand(command="privacy", description="Политика приватности"),
        types.BotCommand(command="forgetme", description="Удалить мои данные"),
    ]
    en = [
        types.BotCommand(command="start", description="Start / restart"),
        types.BotCommand(command="search", description="Find a partner"),
        types.BotCommand(command="profile", description="Profile"),
        types.BotCommand(command="settings", description="Settings"),
        types.BotCommand(command="about", description="About Neverland"),
        types.BotCommand(command="privacy", description="Privacy policy"),
        types.BotCommand(command="forgetme", description="Delete my data"),
    ]
    try:
        ...
        logging.warning(f"set_my_commands fallback failed: {e}")


# ===================== СЛУЖЕБНОЕ: ВЕБ-МОРДА СТАТИСТИКИ =====================
...
def _robust_open_url(url: str) -> bool:
    try:
...
def _wait_and_open_browser(host: str, port: int, path: str = "/", wait_s: int = 20):
    ...
    while time.time() - t0 < wait_s:
        ...
        time.sleep(0.5)
    if _port_is_listening(host, port):
        if _robust_open_url(f"http://{host}:{port}/"):
            logging.info("[stats] Браузер открыт: %s", url)
        else:
            logging.warning("[stats] Не удалось открыть браузер автоматически. Открой вручную: %s", url)
    else:
        logging.warning("[stats] Порт не поднялся: %s", url)


def start_stats_server_in_thread(host: str, port: int, app_module: str = "stats_api:app"):
    def _run():
        try:
            if str(os.getcwd()) not in sys.path:
                sys.path.insert(0, os.getcwd())
            import uvicorn
            config = uvicorn.Config(app_module, host=host, port=port, log_level="info", reload=False)
            server = uvicorn.Server(config)
            logging.info("[stats] Старт на http://%s:%s", host, port)
            server.run()
        except Exception as e:
            logging.warning("[stats] uvicorn failed: %s", e)

    t = threading.Thread(target=_run, daemon=True, name="stats-uvicorn")
    t.start()
    return t
...
async def main():
    ...
    # Веб-морда статистики + авто-открытие вкладки
    if os.getenv("START_STATS", "1") == "1":
        host = os.getenv("STATS_HOST", "127.0.0.1")
        port = int(os.getenv("STATS_PORT", "8000"))
        start_stats_server_in_thread(host, port, "stats_api:app")
        if os.getenv("AUTO_OPEN_STATS", "1") == "1":
            threading.Thread(
